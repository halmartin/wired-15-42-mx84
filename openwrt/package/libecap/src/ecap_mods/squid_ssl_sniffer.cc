...
void Adapter::Service::configure(const libecap::Options &cfg) {
    ...
    if ((iface_fd = open(iface_fpath.c_str(), O_WRONLY)) < 0) {
        throw libecap::TextException(CfgErrorPrefix +
...
int Adapter::Xaction::pushToIoctl(std::string &chunk) const {
    ...
    int ret = write(service->iface_fd, cpy.c_str(), cpy.size());
    // NOTE: we're not handling errors here; failure would imply click is down. We only log for now
    if (ret < 0) {
        msg = "failed to write to click\n"; pushToFile(msg);
    }

    if (fdatasync(service->iface_fd) < 0) {
        msg = "failed to flush click write buffer\n"; pushToFile(msg);
    }
